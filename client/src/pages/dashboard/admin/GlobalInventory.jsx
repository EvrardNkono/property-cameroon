...
// Import des composants uniformisés
...
const GlobalInventory = () => {
  ...
  const inventory = [
    { id: 1, title: "Lotissement Horizon Kribi", location: "Kribi, Zone Balnéaire", price: "15,000,000 FCFA", surface: "500m²", owner: "Samuel E.", status: "PUBLISHED", category: "Terrain" },
    { id: 2, title: "Villa Signature Yassa", location: "Douala, Yassa", price: "85,000,000 FCFA", surface: "350m²", owner: "Marie Ngo", status: "PENDING", category: "Immobilier" },
    { id: 3, title: "Domaine de la Lekié", location: "Obala, Centre", price: "3,500,000 FCFA", surface: "1000m²", owner: "SCI Horizon", status: "SOLD", category: "Terrain" }
  ];
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* HEADER UTILISANT ADMINSECTIONHEADER */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <AdminSectionHeader 
          title="Inventaire Global" 
          description="Gérez l'ensemble des biens et lotissements de la plateforme." 
        />
      ...
      <div className="bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col lg:flex-row gap-4 justify-between items-center">
        <div className="flex gap-2 w-full lg:w-auto overflow-x-auto no-scrollbar">
          {[
            { id: 'ALL', label: 'Tout l\'inventaire' },
        ...
        <div className="relative w-full lg:w-96">
          ...
          <input 
            ...
            placeholder="Rechercher par ville, titre ou proprio..."
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-slate-50/50 border-b border-slate-100">
              <tr>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Produit Immobilier</th>
                <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Localisation</th>
                ...
                <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center text-[#0a2619] border border-slate-100 shadow-sm group-hover:scale-110 transition-transform">
                        {item.category === 'Terrain' ? <MapPin size={22} className="text-[#c5a059]" /> : <Home size={22} className="text-[#0a2619]" />}
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                           <p className="font-bold text-[#0a2619] text-sm">{item.title}</p>
                           {/* Utilisation du Badge Uniformisé */}
                  ...
                  <td className="px-8 py-6 text-right">
                    <div className="flex justify-end gap-2">
                        {/* Utilisation des boutons uniformisés */}
          ...
          {filteredInventory.length === 0 && (
            <div className="p-20 text-center text-slate-400 font-medium italic">Aucun bien trouvé pour cette sélection.</div>
          )}
        </div>
      </div>

      {/* DASHBOARD VALEUR CATALOGUE */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-[#f1f5f9] p-8 rounded-[2.5rem] border border-slate-200">
             <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-4">Répartition de l'offre</h4>
             <div className="flex gap-4">
                 <div className="flex-1 bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
                     ...
                     <p className="text-[9px] font-bold text-slate-400 uppercase">Terrains Nus</p>
                 </div>
                 <div className="flex-1 bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
                     <p className="text-2xl font-black text-[#0a2619]">35%</p>
                     <p className="text-[9px] font-bold text-slate-400 uppercase">Construit</p>
                 </div>
             </div>
          </div>
          <div className="bg-[#0a2619] p-8 rounded-[2.5rem] text-white flex items-center justify-between shadow-2xl relative overflow-hidden group">
            <div className="relative z-10">
                <p className="text-[#c5a059] text-[10px] font-black uppercase tracking-[0.2em] mb-1">Valeur Totale Catalogue</p>
                <h3 className="text-3xl font-black">1.2 Milliards <span className="text-xs font-normal opacity-50 ml-1">FCFA</span></h3>
            </div>
