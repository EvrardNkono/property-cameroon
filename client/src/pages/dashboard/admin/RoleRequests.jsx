...
const RoleRequests = () => {
  ...
  const translations = {
    fr: {
      ...
      noRequestsApproved: "Aucune demande approuvée",
      noRequestsRejected: "Aucune demande rejetée",
      
      // Labels
      currentRoles: "Actuels",
      requestedRoles: "Demandés",
      none: "Aucun",
      reason: "Raison",
      ...
      adminRequired: "Vous devez avoir le rôle ADMIN pour accéder à cette page.",
      currentRoles: "Vos rôles actuels",
    ...
    en: {
      ...
      noRequestsApproved: "No approved requests",
      noRequestsRejected: "No rejected requests",
      ...
      adminRequired: "You must have ADMIN role to access this page.",
  ...
  const handleApprove = async (id) => {
    try {
      ...
      setError(err.response?.data?.message || t.errorApprove);
    } finally {
      setActionId(null);
    }
  };
  ...
  const handleReject = async (id) => {
    try {
      ...
      setError(err.response?.data?.message || t.errorReject);
    } finally {
      setActionId(null);
    }
  };
    ...
    return (
      <div className="p-8 bg-yellow-50 rounded-2xl border-2 border-yellow-200">
        <div className="flex items-center gap-3 text-yellow-700">
          <AlertCircle size={24} />
          <div>
            <h3 className="font-bold">{t.accessRestricted}</h3>
            <p className="text-sm">{t.adminRequired}</p>
            <p className="text-xs mt-2">{t.currentRoles} : {user.roles?.join(', ') || t.none}</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-slate-400">
          ...
          <p className="text-sm">
            {statusFilter === 'PENDING' && t.noRequestsPending}
            {statusFilter === 'APPROVED' && t.noRequestsApproved}
            {statusFilter === 'REJECTED' && t.noRequestsRejected}
          </p>
        </div>
      ) : (
            ...
            <div key={req._id} className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-black text-[#0a2619] text-sm">{req.user?.name || t.unknownUser}</p>
                    <p className="text-[10px] text-slate-500">{req.user?.email || t.emailNotAvailable}</p>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[10px] text-slate-400 uppercase font-bold">{t.currentRoles} :</span>
                  {req.currentRoles?.length > 0 ? req.currentRoles.map(roleBadge) : 
                    <span className="text-[10px] text-slate-400">{t.none}</span>}
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-[10px] text-slate-400 uppercase font-bold">{t.requestedRoles} :</span>
              ...
              <p className="text-[10px] text-slate-400">
                ...
                {req.reviewedBy && ` · ${t.reviewedBy} ${req.reviewedBy.name}`}
              </p>
