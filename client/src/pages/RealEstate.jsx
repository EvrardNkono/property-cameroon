...
// ─────────────────────────────────────────────────────────────
// Hook langue — SYNCHRONE pour éviter le fetch en 'fr' par défaut
// ─────────────────────────────────────────────────────────────
const detectLang = () => {
  ...
  const detected    = urlLang || storedLang || (browserLang === 'en' ? 'en' : 'fr');
  return ['fr', 'en'].includes(detected) ? detected : 'fr';
};

const useCurrentLang = () => {
  // ✅ Initialisation synchrone : la langue est connue AVANT le premier render
  const [lang, setLang] = useState(detectLang);

  useEffect(() => {
    // Réagir si l'utilisateur change la langue dans un autre onglet
...
const RealEstate = () => {
  ...
  // t est recalculé à chaque changement de langue sans ré-fetch si les données
  // sont déjà disponibles (le refetch est géré par le useEffect ci-dessous)
  ...
  // Grâce à useState(detectLang), la bonne langue est utilisée dès le PREMIER fetch.
  useEffect(() => {
    fetchProperties();
  }, [currentLang]);
  ...
  const fetchProperties = async () => {
    try {
      ...
      // ✅ lang est toujours la valeur correcte dès le premier appel
