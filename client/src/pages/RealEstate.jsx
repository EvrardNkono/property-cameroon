...
// Avant : useState('fr') puis useEffect → premier fetch toujours en 'fr'
// Maintenant : useState(detectLang) → la bonne langue dès le premier render
// ─────────────────────────────────────────────────────────────
const detectLang = () => {
  ...
  // Tes données source sont en anglais, les langues supportées sont en/fr
  return ['fr', 'en'].includes(detected) ? detected : 'en';
};

const useCurrentLang = () => {
  const [lang, setLang] = useState(detectLang); // ✅ synchrone, pas de valeur par défaut incorrecte

  useEffect(() => {
    // Réagit si l'utilisateur change la langue dans un autre onglet
...
// ─────────────────────────────────────────────────────────────
// Traductions de l'interface (labels, boutons, placeholders)
// Les données propriétés elles sont traduites côté backend
// ─────────────────────────────────────────────────────────────
const TRANSLATIONS = {
  en: {
...
const RealEstate = () => {
  ...
  // ─── Fetch : se déclenche à chaque changement de langue ──
  // Grâce à useState(detectLang), currentLang est correct dès le PREMIER appel
  useEffect(() => {
    fetchProperties();
  }, [currentLang]);
  ...
  const fetchProperties = async () => {
    try {
      setLoading(true);
      setError(null);
      // ✅ lang est toujours correct dès le premier appel — plus de fetch en 'fr' par défaut
      ...
      else if (response.data && Array.isArray(response.data)) propertiesData = response.data;
  ...
  const applyFilters = () => {
    ...
    if (filters.minPrice)              filtered = filtered.filter(p => p.price?.amount >= parseInt(filters.minPrice));
    if (filters.maxPrice)              filtered = filtered.filter(p => p.price?.amount <= parseInt(filters.maxPrice));
    if (filters.region !== 'all')      filtered = filtered.filter(p => p.location?.region === filters.region);
  ...
  const resetFilters = () => {
    setFilters({ category: 'all', listingType: 'all', minPrice: '', maxPrice: '', region: 'all', bedrooms: 'all', furnished: 'all' });
    setSearchTerm('');
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <section className="relative bg-gradient-to-r from-emerald-900 to-emerald-800 text-white pt-24 pb-16">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="text-emerald-100 text-lg max-w-2xl mx-auto">{t.heroSubtitle}</p>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {[
                      ...
                      <button
                        key={opt.id}
                        onClick={() => setFilters({ ...filters, furnished: opt.id })}
