...
const FilterBar = ({ onFilterChange, initialFilters = {} }) => {
  ...
  // ✅ Ref pour éviter la boucle infinie causée par onFilterChange dans les deps
  const onFilterChangeRef = useRef(onFilterChange);
  useEffect(() => { onFilterChangeRef.current = onFilterChange; }, [onFilterChange]);

  useEffect(() => {
    if (onFilterChangeRef.current) {
      onFilterChangeRef.current({ category, location, maxBudget, listingType });
    }
  }, [category, location, maxBudget, listingType]); // ✅ onFilterChange retiré des dépendances
  ...
  return (
    // ✅ z-10 au lieu de z-20, et mx-4 sm:mx-6 pour rester dans le flow
    <div className="max-w-7xl mx-auto bg-white p-4 rounded-xl shadow-2xl -mt-10 relative z-10 border border-slate-100 mx-4 sm:mx-6 lg:mx-auto">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="flex flex-col">
          ...
          <input
            ...
            className="bg-slate-100 text-slate-900 text-xs font-medium p-3 rounded-lg outline-none placeholder:text-slate-400 focus:ring-2 focus:ring-emerald-500 transition-all"
          />
        </div>
      </div>
      ...
      <div className="flex gap-3 mt-4">
        <button
          onClick={() => onFilterChangeRef.current?.({ category, location, maxBudget, listingType })}
